              ...
              <div className="relative">
                ...
                >
                  <div className="w-7 h-7 rounded-lg bg-blue-600 text-white font-bold text-xs flex items-center justify-center">
                    {(session.profile?.full_name || session.user?.email || 'U').charAt(0).toUpperCase()}
                  </div>
                  <div className="hidden sm:flex flex-col text-start">
                    <span className="text-xs font-bold text-slate-900 leading-none">
                      {(session.profile?.full_name || session.user?.email || 'مستخدم').split(' ')[0]}
                    </span>
                ...
                {showUserMenu && (
                  ...
                  >
                    <div className="px-4 py-2 border-b border-slate-100">
                      <p className="text-xs font-bold text-slate-900">{session.profile?.full_name || 'مدير الحساب'}</p>
